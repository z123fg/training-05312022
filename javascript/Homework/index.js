//Question 1:
function qOne(input) {
  if (!isNaN(input)) return input.split("").reverse().join("");
  else return "Please input number.";
...
function qTwo(input) {
  ...
  if (reversed === input.replace(" ", "")) return true;
  else return false;
}
...
function qTSix(input) {
  ...
  for (var i = 0; i < input.length; i++) {
    if (output.indexOf(input.charAt(i)) == -1) output += input[i];
  }
  return output;
}
...
function qTEight(input, key) {
  ...
  while (r >= l) {
    ...
    if (input[mid] == key) return "The index of the search is " + mid;

    if (input[mid] > key) r = mid - 1;
    else l = mid + 1;
  }
...
function qTNine(input, key) {
  var output = [];
  for (let i = 0; i < input.length; i++) {
    if (input[i] >= key) output.push(input[i]);
  }

  if (output.length > 1) return output;
  else return "No element in the list is larger than the key.";
}
...
function qTwThree(input) {
  ...
  else return "There is no first not repeated character in the string.";
}
...
console.log(
  "Question 24: " +
    qTwFour([12, 345, 4, 546, 122, 84, 98, 64, 9, 1, 3223, 455, 23, 234, 213])
);
...
function qTwFive(input) {
  ...
  for (let i = 0; i < input.length - 1; i++) {
    if (input[i].length > input[i + 1].length) output = input[i];
...
console.log(
  "Question 25: " +
    qTwFive(["Australia", "Germany", "United States of America"])
);
...
//Question findMedianSortedArrays:
function findMedianSortedArrays(nums1, nums2) {
  ...
  var median;
  if (arr.length % 2 !== 0) {
    let index = (0 + arr.length - 1) / 2;
    median = arr[index];
  } else {
    let a = (0 + arr.length) / 2;
    let b = a - 1;
...
console.log("Question 30: " + findMedianSortedArrays([-1, -2], [2]));
