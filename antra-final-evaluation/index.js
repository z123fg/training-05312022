...
const View = (() => {
    ...
    const renderTodolist = (todos) => {
        ...
        todos.forEach((todo) => {
            ...
            template += `
                <li id="${todo.id}">
                    <input type=text class="${todo.completed ? 'todo-input strike' : 'todo-input'}" value=${todo.content} readonly>
    ...
    const completeTodo = () => {
        View.todoListEl.addEventListener("click", (event) => {
            ...
            if (event.target.className.includes("todo-input") && editButton.innerHTML !== 'Save') {
                const id = li.id;
                const todo = state.todos.find((el) => el.id.toString() === id);
                const newTodo = { ...todo, completed: !todo.completed };
                APIs.editTodo(li.id, newTodo).then(res => {
                })
            }
        })
    }
